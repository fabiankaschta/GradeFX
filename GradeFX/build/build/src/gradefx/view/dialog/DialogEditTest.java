...
public class DialogEditTest extends DialogEdit<Test> {
	...
	public DialogEditTest(Group group, Test test) {
		super(Translator.get("dialog_edit_test_title"), test);
		this.group = group;
		...
		super.addInput(this.date, Translator.get("test_date"));
		...
		super.setCustomButtonDisableExpression(this.totalPoints,
				this.totalPoints.visibleProperty().and(this.totalPoints.isSelectedExpression())
						.and(this.totalPoints.isSelectedDefaultExpression().not())
						.and(this.totalPoints.valueProperty().isNotEqualTo(BigDecimal.ZERO)));
		// FIXME still possible to enter 0 points here
