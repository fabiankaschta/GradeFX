...
public class TestStatisticsSidePane extends ScrollPane {
	...
	public TestStatisticsSidePane(Group group, Test test) {
		...
		// FIXME vertical scroll bar should make this wider when showing
		...
		TableViewPointsSystem tableViewPointsSystem = new TableViewPointsSystem(group, test);
		VBox headerBox = new VBox(10, header, tableViewPointsSystem);
		...
		avgLabel.textProperty().bind(Bindings.createStringBinding(() -> {
			String avgPrefix = Translator.get("test_avg") + ": ";
			BigDecimal avg = tableViewPointsSystem.getGradeAVG();
			if (avg == null) {
				return avgPrefix + '-'; // '\u2014'; // long dash
			} else {
				return avgPrefix + converter.toString(avg);
			}
		...
		settingsPane.setText(Translator.get("pointsSystem_settings"));
		...
		settingsPane.setContent(settingsPaneContent);
		settingsPane.visibleProperty().bind(test.usePointsProperty());
		content.getChildren().add(settingsPane);

		TitledPane sliderPane = new TitledPane();
		sliderPane.setText(Translator.get("pointsSystem_silder"));
		...
		sliderPane.setContent(sliderPaneContent);
		sliderPane.visibleProperty().bind(test.usePointsProperty());
		content.getChildren().add(sliderPane);
	...
	private class RatioSlider extends HBox {
		...
		private RatioSlider(Grade grade, BigDecimal value) {
			...
			this.slider.setPrefWidth(75); // FIXME slider should resize to fill space in sliderBox
