...
public class Test {
	...
	public Test(Group group, String name, LocalDate date, BigDecimal weight, BigDecimal maxPoints, boolean useTasks,
			boolean usePoints) {
		this(name, date, weight, useTasks, usePoints);
	...
	private Test(String name, LocalDate date, BigDecimal weight, boolean useTasks, boolean usePoints) {
		this.setName(name);
		this.setDate(date);
	...
	private void putStudentPropertiesIfNotExists(Student student) {
		if (!this.grades.containsKey(student)) {
			...
			gradeProperty
					.addListener(Controller.getConditionalListenerUnsavedChanges(() -> this.isGradeFixed(student)));
			gradeFixedProperty.addListener(Controller.LISTENER_UNSAVED_CHANGES);
			totalPointsProperty.addListener(
					Controller.getConditionalListenerUnsavedChanges(() -> this.isTotalPointsFixed(student)));
	...
	public ObservableMap<Student, ObjectProperty<Grade>> getGrades() {
		// FIXME for some reasons this does not report adding new elements correctly
//		return FXCollections.unmodifiableObservableMap(this.grades);
		return this.grades;
	}
	...
	private static class TestS implements DataObject<Test> {
		...
		public Test deserialize(Object... params) {
			if (test == null) {
				test = new Test(name, date, weight, useTasks, usePoints);
